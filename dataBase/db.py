...
class DBAPI:
    ...
    def add_password(self, user_id : int, service : str, login : str, password :str):
        self.connect()  # Установка соединения с базой данных

        # Выполнение операции добавления пароля в базу данных
        self.conn.execute("INSERT INTO passwords (user_id, service, login, password) VALUES (?, ?, ?, ?)",
    ...
    def get_password(self, user_id : int, service : str, login : str):
        ...
        cursor.execute("SELECT login, password FROM passwords WHERE user_id=? AND service=? AND login=?",
                    (user_id, service, login))
        result = cursor.fetchone()
        self.disconnect()
        return result
    
    def update_password(self, user_id : int, service : str, login : str, password : str):
        self.connect()

        self.delete_password(user_id, service, login)
        self.add_password(user_id, service, login, password)
